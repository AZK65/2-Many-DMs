...
export function Board() {
  ...
  async function move(contactId: string, targetCol: string) {
    ...
    // Optimistic: drop the source tag, add the target tag.
    setContacts((prev) =>
      prev.map((c) => {
    ...
    <div className="flex h-screen flex-col bg-slate-100">
      ...
      <header className="flex items-center justify-between border-b border-slate-200 bg-white px-5 py-3">
        ...
        <div className="text-xs text-slate-400">
          {contacts.length} contacts · drag a card to change its stage
        </div>
      </header>
          ...
          <div className="flex h-full items-start gap-4 p-4">
            {columns.map((col) => (
              <div
                ...
                className={`flex max-h-full w-72 shrink-0 flex-col rounded-xl border bg-slate-50 transition ${
                  overCol === col.id
                  ...
                  {col.cards.map((card) => {
                    ...
                    return (
                      ...
                      >
                        ...
                        <div className="mt-1 flex items-center gap-1 pl-[42px] text-[10px] text-slate-400">
                          <span
